...
// Perceptual colour enhancement using audio psychoacoustic parameters
// Brightness modulated by loudness (ITU-R BS.1770 K-weighted), energy, crest factor, and onset detection
// Saturation reduced by spectral flatness (noise-like signals → desaturated)
// Hue shifted based on spectral centroid (timbral brightness) and rolloff (spectral energy distribution)
//
// Enhancement parameter values were determined through:
// - Perceptual experiments matching audio characteristics to colour perception
// - Balancing visibility across dynamic range (quiet passages must remain visible)
// - Preventing over-saturation that would cause gamut clipping
// - Ensuring smooth temporal transitions (avoiding jarring colour changes)
void ColourMapper::enhanceColourWithAudioParams(
	...
	const float clampedLoudness = std::clamp(loudness, 0.0f, 1.0f);
	const float clampedFlatness = std::clamp(spectralFlatness, 0.0f, 1.0f);
	const float clampedCentroid = std::clamp(spectralCentroid, CENTROID_MIN_FREQ, CENTROID_MAX_FREQ);
	const float clampedSpread = std::clamp(spectralSpread / SPREAD_NORMALISATION, 0.0f, 1.0f);
	const float clampedEnergy = std::clamp(totalEnergy / ENERGY_NORMALISATION, 0.0f, 1.0f);
	const float clampedRolloff = std::clamp(spectralRolloff, ROLLOFF_MIN_FREQ, ROLLOFF_MAX_FREQ);
	const float clampedCrest = std::clamp(spectralCrestFactor, CREST_MIN, CREST_MAX);
	const float clampedFlux = std::clamp(spectralFlux / FLUX_NORMALISATION, 0.0f, 1.0f);

	float L, a, b_comp;
	RGBtoLab(r, g, b, L, a, b_comp, colourSpace);

	L = std::max(L, 0.0f);

	// Brightness modulation based on audio parameters
	float brightnessScale = LOUDNESS_MIN_BRIGHTNESS + LOUDNESS_BRIGHTNESS_RANGE * clampedLoudness;
	brightnessScale *= (1.0f + ENERGY_BRIGHTNESS_BOOST * clampedEnergy);

	const float crestNormalised = (clampedCrest - CREST_MIN) / (CREST_MAX - CREST_MIN);
	brightnessScale *= (1.0f + CREST_BRIGHTNESS_BOOST * crestNormalised);
	brightnessScale *= (1.0f + FLUX_BRIGHTNESS_BOOST * clampedFlux);

	if (onsetDetected) {
		brightnessScale *= (1.0f + ONSET_BRIGHTNESS_FLASH);
	}

	L *= brightnessScale;

	// Saturation modulation based on spectral characteristics
	float chromaScale = 1.0f - (FLATNESS_SATURATION_REDUCTION * clampedFlatness);
	chromaScale *= (1.0f - SPREAD_SATURATION_REDUCTION * clampedSpread);

	// Hue shifting based on spectral centroid and rolloff
	constexpr float DEG_TO_RAD = std::numbers::pi_v<float> / 180.0f;

	const float centroidNormalised = (clampedCentroid - CENTROID_MIN_FREQ) /
									 (CENTROID_MAX_FREQ - CENTROID_MIN_FREQ);
	float hueShiftDegrees = (centroidNormalised - 0.5f) * 2.0f * CENTROID_HUE_SHIFT_DEGREES;

	const float rolloffNormalised = (clampedRolloff - ROLLOFF_MIN_FREQ) /
									(ROLLOFF_MAX_FREQ - ROLLOFF_MIN_FREQ);
	hueShiftDegrees += (rolloffNormalised - 0.5f) * 2.0f * ROLLOFF_HUE_SHIFT_DEGREES;

	const float hueShiftRadians = hueShiftDegrees * DEG_TO_RAD;

	const float chroma = std::sqrt(a * a + b_comp * b_comp);

	if (chroma > EPSILON_SMALL) {
		const float currentHue = std::atan2(b_comp, a);
		const float newHue = currentHue + hueShiftRadians;

		const float scaledChroma = chroma * chromaScale;
		a = scaledChroma * std::cos(newHue);
		b_comp = scaledChroma * std::sin(newHue);
	} else {
		a *= chromaScale;
		b_comp *= chromaScale;
	}
	...
	LabtoRGB(L, a, b_comp, r, g, b, colourSpace);
