...
// Volk headers (if needed for dynamic loading)
...
void windowStyling(GLFWwindow* window) {
    // Linux window styling - basic setup for now
    // Could be extended with X11/Wayland specific theming
...
int app_main(int, char**)
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        if (!is_minimized)
        {
            wd->ClearValue.color.float32[0] = clear_colour[0];
            wd->ClearValue.color.float32[1] = clear_colour[1];
            wd->ClearValue.color.float32[2] = clear_colour[2];
            wd->ClearValue.color.float32[3] = clear_colour[3];
